package com.icecream.order.config.sharding;
...
public class DataSourcePool {
    ...
    private DataSource buildDataSource() {
        ...
        dataSourceMap.put("xgqoms", createDataSource("xgqoms"));
        //设置默认db为ds_0，也就是为那些没有配置分库分表策略的指定的默认库
        //如果只有一个库，也就是不需要分库的话，map里只放一个映射就行了，只有一个库时不需要指定默认库，但2个及以上时必须指定默认库，否则那些没有配置策略的表将无法操作数据
        DataSourceRule dataSourceRule = new DataSourceRule(dataSourceMap, "xgqoms");
        ...
        TableRule orderTableRule = TableRule.builder("order_info")
                .actualTables(Arrays.asList("order_info_0001", "order_info_0002"))
    ...
    private static DataSource createDataSource(final String dataSourceName) {
        ...
        result.setUrl(String.format("jdbc:mysql://10.40.254.59:4000/%s", dataSourceName));
